  ...
  const handlePickFolder = useCallback(async () => {
    ...
    openPicker({ pickFolder: true, multiple: false }, async (folders) => {
      ...
      try {
        ...
        const supportedFiles = folderContents.filter(file => 
        ...
        toast.success(`Loaded ${filesToProcess.length} file(s) from ${folder.name}`);
      } catch (error) {
        console.error('Error fetching folder contents:', error);
        toast.error(`Error loading files from folder: ${error instanceof Error ? error.message : 'Unknown error'}`);
      } finally {
        setProcessingStatus({ 
          isProcessing: false, 
          currentStep: '', 
    ...
    try {
      ...
      for (let i = 0; i < selectedFiles.length; i++) {
        ...
        try {
          ...
          fileContents.push(`### ${file.name} (ID: ${file.id})\n${truncatedContent}`);
        } catch (error) {
      ...
      <Card className="w-full shadow-lg">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-2xl">Google Drive AI Analyzer</CardTitle>
              <CardDescription>
              ...
              <div className="space-y-6">
                ...
                <div>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <Badge variant={selectedFiles.length > 0 ? "default" : "outline"}>
                  ...
                  {selectedFiles.length > 0 ? (
                    <ScrollArea className="h-48 border rounded-md p-2">
                      <ul className="space-y-1">
                        {displayFiles.map((file) => (
                          <li key={file.id} className="flex items-center gap-2 p-1 hover:bg-muted/50 rounded">
                        ...
                        {selectedFiles.length > displayFiles.length && (
                          <li className="text-center text-sm text-muted-foreground pt-2">
                            + {selectedFiles.length - displayFiles.length} more files
                          </li>
                        )}
                      </ul>
                    </ScrollArea>
                    ...
                    <div className="h-48 border rounded-md flex items-center justify-center text-muted-foreground">
                      No files selected. Use the buttons above to select files or a folder.
                    </div>
                  )}
                </div>
                ...
                <div className="grid gap-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <Label htmlFor="maxFiles">Max Files (for folder selection)</Label>
                      <Input
                        id="maxFiles"
                        type="number"
                        min="1"
                  ...
                  <div className="flex items-center space-x-2">
                    <Checkbox 
                      ...
                      onCheckedChange={(checked) => setIncludeSubfolders(!!checked)} 
                    />
                    <Label htmlFor="includeSubfolders">Include Subfolders (when selecting folder)</Label>
                  </div>
                </div>
                ...
                <Alert variant="default" className="bg-muted/50">
                  ...
                  <AlertDescription>
                    Files will be processed up to {MAX_DOC_CHARS.toLocaleString()} characters each.
                    When selecting a folder, up to {maxFiles} most recent files will be processed.
                  </AlertDescription>
                </Alert>
              </div>
            </TabsContent>
                ...
                {processingStatus.isProcessing && (
                  <div className="space-y-2 mb-4">
                    <div className="flex justify-between">
                      ...
                      <span>
                        {processingStatus.processedFiles} / {processingStatus.totalFiles} files
                      </span>
                    </div>
        ...
        <CardFooter className="flex flex-col sm:flex-row gap-4 justify-end border-t p-4">
          <Button
            ...
            disabled={!isSignedIn || !isReady || selectedFiles.length === 0 || processingStatus.isProcessing}
